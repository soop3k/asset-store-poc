...
import com.db.assetstore.mapper.AttributeMapper;
...
public class JpaAssetRepository implements AssetRepository {
    ...
    /**
     * Persists the asset and enforces attribute invariants based on AttributeDefEntity for the asset type.
     * Validations performed here (repository-level guardrails):
     * - Every provided attribute name must be defined for the asset type (derived from JSON Schemas at startup).
     * - All attributes marked as required must be present in the payload.
     * These checks protect data integrity even if upper layers skip or relax schema validation.
     */
    @Override
    ...
    public String saveAsset(Asset asset) {
        ...
        try {
            // Ensure a transaction when running outside Spring-managed context (e.g., plain JPA tests)
            ...
            if (defs == null || defs.isEmpty()) {
                // Fallback to in-memory definitions derived from JSON Schemas if DB is not yet bootstrapped
                Map<String, AttributeDefinitionRegistry.Def> regDefs = AttributeDefinitionRegistry.getInstance().getDefinitions(e.getType());
                Map<String, AttributeDefEntity> tmp = new HashMap<>();
                for (var entry : regDefs.entrySet()) {
                    var d = entry.getValue();
                    tmp.put(entry.getKey(), new AttributeDefEntity(e.getType(), d.name(), d.valueType().dbName(), d.required()));
                }
                defs = tmp;
            }
            Set<String> provided = new HashSet<>();
            asset.attributes();
            if (!asset.attributes().isEmpty()) {
                ...
                for (AttributeValue<?> av : asset.attributes().values()) {
                    ...
                    AttributeEntity cur = AttributeMapper.toEntity(e, av, now);
            ...
            for (AttributeDefEntity def : defs.values()) {
                if (def.isRequired() && !provided.contains(def.getName())) {
                    log.warn("Rejecting asset save: missing required attribute '{}' for type {} (id={})", def.getName(), e.getType(), e.getId());
    ...
    public List<AttributeHistory> history(String assetId) {
        ...
        for (Object[] r : rows) {
            out.add(new AttributeHistory((String) r[0], (String) r[1], (String) r[2], (String) r[3], (Instant) r[4]));
        }
        return out;
    }
    ...
    private Map<String, AttributeValue<?>> loadAttributes(String assetId) {
        ...
        for (AttributeEntity r : rows) {
            AttributeValue<?> av = AttributeMapper.toModel(r);
            map.put(av.name(), av);
        }
        return map;
    }
    ...
    private static void ensureDefinedFull(Map<String, AttributeDefEntity> defs, String name) {
        if (defs == null || !defs.containsKey(name)) {
            // This guards DB integrity: we only persist attributes that have a definition for the asset type
            log.warn("Rejecting asset save: attribute '{}' is not defined for this type", name);
    ...
    private Predicate attrPredicate(Condition c, Join<AssetEntity, AttributeEntity> join, CriteriaBuilder cb) {
        ...
        Predicate valuePred = switch (op) {
            case EQ -> cb.equal(join.get("value"), AttributeMapper.stringify(val));
            case LIKE -> cb.like(join.get("value"), AttributeMapper.stringify(val));
